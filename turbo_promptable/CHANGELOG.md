...
### Changed
- **BREAKING** `TTriggerKind.name` is renamed to `id`. Trigger settings name every kind with `id:`. `TTriggerContents.kind` holds the kind id
- `TStreamTriggerKind` can be held by several claims at the same time
