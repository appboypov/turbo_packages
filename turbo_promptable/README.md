...
`TTriggerSettings` holds the plx trigger settings: a list of `TTriggerKind`s. Each kind has a unique `id` and a pattern (`start`, optional `contains`, `end`). The optional `ignore` list holds gitignore-style rules relative to each watched folder; plx skips the files and folders they exclude.
