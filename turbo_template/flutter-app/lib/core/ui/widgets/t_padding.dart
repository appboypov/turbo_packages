...
class TPadding extends EdgeInsets {
  ...
  const TPadding.left({double value = TSizes.appPadding})
    : super.only(left: value);

  const TPadding.right({double value = TSizes.appPadding})
    : super.only(right: value);

  const TPadding.top({double value = TSizes.appPadding})
    : super.only(top: value);

  const TPadding.bottom([double value = TSizes.appPadding])
    : super.only(bottom: value);

  const TPadding.horizontal([double value = TSizes.appPadding])
    : super.symmetric(horizontal: value);

  const TPadding.vertical({double value = TSizes.appPadding})
